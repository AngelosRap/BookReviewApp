...
public class ReviewsController(IReviewService reviewService) : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] ReviewCreateRequest reviewCreateRequest)
    {
        ...
        var reviewRes = await _reviewService.Create(review);

        if (reviewRes.Failed)
        {
            return BadRequest(reviewRes.Message);
        }

        var response = reviewRes.Data!.ToResponse();
    ...
    public async Task<IActionResult> Vote(int id, [FromBody] ReviewVoteRequest reviewVote)
    {
        ...
        var reviewRes = await _reviewService.Get(id);
        if (reviewRes.Failed)
        {
            return NotFound(reviewRes.Message);
        }
